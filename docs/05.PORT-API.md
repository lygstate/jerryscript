...
The `jerry_port_print_string` is currenlty not used by the jerry-core directly.
However, it provides a port specifc way for `jerry-ext` components to print
information.

```c
/**
 * Print a utf8 string to stdout.
 */
 ...
 */
void
jerry_port_print_string (const char *s, size_t len)
{
