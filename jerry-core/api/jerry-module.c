 ...
 */
static jerry_size_t
jerry_path_join_and_normalize (jerry_path_style_t path_style, /**< The style of the path list */
                               bool is_resolve, /**< Join path in resolve mode */
                               bool remove_trailing_slash, /**< If remove the trailing slash symbol */
                               const jerry_string_t *path_list_p, /**< Path list */
                               jerry_size_t path_list_count, /**< Path list count */
                               jerry_char_t *buffer_p, /**< The buffer to storaged the joined path */
                               jerry_size_t buffer_size /**< The size of the buffer_p */
)
{
...
{
  jerry_char_t *buffer_p = NULL;
  jerry_size_t path_size = jerry_path_join_and_normalize (style, true, true, path_list_p, path_list_count, NULL, 0);
  if (path_size > 0)
  {
    jerry_size_t buffer_size = path_size + 1;
    buffer_p = jerry_heap_alloc (buffer_size);
    if (buffer_p)
    {
      if (jerry_path_join_and_normalize (style, true, true, path_list_p, path_list_count, buffer_p, buffer_size)
          != path_size)
      {
        jerry_heap_free (buffer_p, buffer_size);
        buffer_p = NULL;
